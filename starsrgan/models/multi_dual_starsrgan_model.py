...
from basicsr.data.degradations import random_add_gaussian_noise_pt_info, random_add_poisson_noise_pt_info
from basicsr.data.degradations import only_generate_gaussian_noise_pt_info, only_generate_poisson_noise_pt_info
...
class MultiDualStarSRGANModel(StarGANModel):
    ...
    def feed_data(self, data_all):
        ...
        if self.is_train:
            ...
            if self.degradation_degree == 'severe_degrade_two_stage':
                self.degradation_params = torch.zeros(self.opt_train['batch_size_per_gpu'], self.num_degradation_params)
                ...
                if np.random.uniform() < self.opt['gaussian_noise_prob']:
                    sigma, gray_noise, out, self.noise_g_first = random_add_gaussian_noise_pt_info(
                    ...
                    scale, gray_noise, out, self.noise_p_first = random_add_poisson_noise_pt_info(
                        out,
                ...
                if np.random.uniform() < self.opt['gaussian_noise_prob2']:
                    sigma, gray_noise, out, self.noise_g_second = random_add_gaussian_noise_pt_info(
                    ...
                    scale, gray_noise, out, self.noise_p_second = random_add_poisson_noise_pt_info(
                        out,
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(MultiDualStarSRGANModel, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
