...
class StarSRGANDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.kernel_range = [
            2 * v + 1
            for v in range(math.ceil(self.blur_kernel_size_minimum / 2), math.ceil(self.blur_kernel_size / 2))
        ]
        self.pulse_tensor = torch.zeros(21, 21).float()  # convolving with pulse tensor brings no blurry effect
        self.pulse_tensor[10, 10] = 1
        ...
        self.kernel_range2 = [
            2 * v + 1
            for v in range(math.ceil(self.blur_kernel_size2_minimum / 2), math.ceil(self.blur_kernel_size2 / 2))
        ]
    ...
    def __getitem__(self, index):
        ...
        if np.random.uniform() < self.opt['final_sinc_prob']:
            ...
            kernel_sinc_info = {'kernel': sinc_kernel, 'kernel_size': kernel_size, 'omega_c': omega_c}  # DASR
        else:
